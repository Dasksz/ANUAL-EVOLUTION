...
ON public.cache_filters (ano, mes, filial, cidade, superv, nome, codfor);
...
BEGIN
    ...
    FROM (
        SELECT filial, cidade, superv, nome, codfor, fornecedor, dtped FROM public.data_detailed
        UNION ALL
        SELECT filial, cidade, superv, nome, codfor, fornecedor, dtped FROM public.data_history
    ) t

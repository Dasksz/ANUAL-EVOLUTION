...
BEGIN
    ...
    IF v_is_month_filtered THEN
        ...
        BEGIN
            ...
            -- PREFIX ALL COLUMNS WITH 's.' TO AVOID AMBIGUITY WITH CLIENTS TABLE
            ...
            v_sql := v_sql || ',
            daily_raw AS (
                SELECT s.dtped, s.vlvenda, s.totpesoliq, s.bonificacao, s.tipovenda
                FROM public.data_detailed s
                LEFT JOIN public.dim_produtos dp ON s.produto = dp.codigo
                LEFT JOIN public.data_clients c ON s.codcli = c.codigo_cliente -- Need clients for Rede logic
                ' || v_raw_where || '
                AND EXTRACT(MONTH FROM s.dtped) = $3
                AND EXTRACT(YEAR FROM s.dtped) IN ($2, $4)

...
BEGIN
    ...
    client_agg AS (
        SELECT
            ...
            SUM(CASE
                ...
                ELSE 0
            END) as cli_faturamento,

            SUM(CASE WHEN tipovenda NOT IN (''5'', ''11'') THEN vlvenda ELSE 0 END) as cli_total_sold_base,

            -- Peso Logic
            SUM(CASE
                WHEN ($1 IS NOT NULL AND COALESCE(array_length($1, 1), 0) > 0 AND $1 <@ ARRAY[''5'',''11'']) THEN
                     CASE WHEN tipovenda = ANY($1) THEN peso ELSE 0 END
                ELSE
                    CASE
                        WHEN ($1 IS NOT NULL AND COALESCE(array_length($1, 1), 0) > 0) THEN
                             CASE WHEN tipovenda = ANY($1) AND tipovenda NOT IN (''5'', ''11'') THEN peso ELSE 0 END
                        WHEN tipovenda NOT IN (''5'', ''11'') THEN peso
                        ELSE 0
                    END
            END) as cli_peso,
            ...
            END) as cli_bonificacao,

            -- Devolucao
            SUM(CASE
                WHEN ($1 IS NOT NULL AND COALESCE(array_length($1, 1), 0) > 0) THEN
                    CASE WHEN tipovenda = ANY($1) THEN devolucao ELSE 0 END
                ELSE devolucao
    ...
    SELECT
        COALESCE((SELECT val FROM kpi_active_count), 0),
        COALESCE((SELECT val FROM kpi_base_count), 0),
        COALESCE(json_agg(json_build_object(
            ''month_index'', a.mes - 1,
            ''faturamento'', a.faturamento,
            ''total_sold_base'', a.total_sold_base,

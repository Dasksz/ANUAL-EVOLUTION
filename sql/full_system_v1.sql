...
BEGIN
    ...
    -- Standard Tri Logic (Last 3 months excluding current target month, OR if target is month, 3 months prior?)
    -- Usually "Quarter" KPI means "Previous Quarter" or "Rolling 3 Months".
    -- Logic in app.js for other views: Month vs Trim. Ant.
    -- Here we set reference dates.
    v_tri_end := (v_ref_date - interval '1 day')::date;
    v_tri_start := (v_ref_date - interval '3 months')::date;

    -- 1.1. Trend Logic Calculation
    SELECT MAX(dtped)::date INTO v_max_sale_date FROM public.data_detailed;
    IF v_max_sale_date IS NULL THEN v_max_sale_date := CURRENT_DATE; END IF;
    ...
    IF v_use_cache THEN
        ...
        EXECUTE format('
            ...
            -- Trend Month Data (Specific Month for Calculation)
            kpi_trend_month AS (
                SELECT
                    SUM(vlvenda) as fat,
                    SUM(peso) as peso,
            ...
            SELECT
                ...
                (SELECT row_to_json(tm) FROM kpi_trend_month tm)
        ',
        ...
        v_where_summary, v_current_year, (v_curr_month_idx + 1), -- KPI Trend Month (Using calculated index from trend logic)
        ...
        INTO v_chart_data, v_kpis_current, v_kpis_previous, v_kpis_tri_avg, v_products_table, v_trend_data;
    ...
    ELSE
        ...
        EXECUTE format('
            ...
            kpi_trend_month AS (
                SELECT
                    ...
                    SUM(COALESCE(qtvenda_embalagem_master, 0)) as caixas
                FROM base_data
                WHERE EXTRACT(YEAR FROM dtped) = %L AND EXTRACT(MONTH FROM dtped) = %L
            ),
            prod_agg AS (
                SELECT
                    produto,
        ...
        v_current_year, (v_curr_month_idx + 1), -- Trend Month
        v_current_year, CASE WHEN v_target_month IS NOT NULL THEN format(' AND EXTRACT(MONTH FROM dtped) = %L ', v_target_month) ELSE '' END
        )
        INTO v_chart_data, v_kpis_current, v_kpis_previous, v_kpis_tri_avg, v_products_table, v_trend_data;
    END IF;

    -- Apply Trend Factor to Trend Data
    IF v_trend_allowed AND v_trend_data IS NOT NULL THEN
        -- Safely cast to numeric before multiplying
        -- The row_to_json returns json object, we need to manipulate it or cast it
        -- Let's reconstruct it
        v_trend_data := json_build_object(
            'fat', (v_trend_data->>'fat')::numeric * v_trend_factor,
            'peso', (v_trend_data->>'peso')::numeric * v_trend_factor,
            'caixas', (v_trend_data->>'caixas')::numeric * v_trend_factor,
            'month_index', v_curr_month_idx
        );
    ELSE
        v_trend_data := null;

-- Fix for PGRST203: Could not choose the best candidate function (Ambiguity/Overloading)
-- We explicitly drop potential overlapping signatures before re-creating the definitive one.

-- 1. Drop the current full signature (9 args)
DROP FUNCTION IF EXISTS public.get_dashboard_filters(text[], text[], text[], text[], text[], text, text, text[], text[]);

-- 2. Drop potential legacy signature (8 args - without p_rede)
DROP FUNCTION IF EXISTS public.get_dashboard_filters(text[], text[], text[], text[], text[], text, text, text[]);

-- 3. Drop potential older signature (7 args - without p_tipovenda, p_rede)
DROP FUNCTION IF EXISTS public.get_dashboard_filters(text[], text[], text[], text[], text[], text, text);

-- 4. Re-create the function (Latest version with product sales existence check)

...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function loadData() {
    try {
      ...
      // Initialize Categorias Slide
      let activeMetric = "geral";
      renderCategoriasDispute(rpcData, activeMetric);
  ...
  async function initLojaPerfeitaSlide() {
    const urlParams = new URLSearchParams(window.location.search);
    const p_ano = parseInt(urlParams.get("ano") || new Date().getFullYear());
    const p_mes = parseInt(urlParams.get("mes") || new Date().getMonth() + 1);

    const supFilter = document.getElementById("loja-perfeita-supervisor-filter");
    const pesqFilter = document.getElementById("loja-perfeita-pesquisador-filter");
    const loadingEl = document.getElementById("loja-perfeita-loading");

    // Populate filters
    const { data: filterData } = await supabase.from("data_nota_perfeita").select("supervisor, pesquisador");
    if (filterData) {
      const sups = [...new Set(filterData.map(d => d.supervisor).filter(Boolean))].sort();
      const pesqs = [...new Set(filterData.map(d => d.pesquisador).filter(Boolean))].sort();

      sups.forEach(s => supFilter.add(new Option(s, s)));
      pesqs.forEach(p => pesqFilter.add(new Option(p, p)));
    }

    const loadData = async () => {
      loadingEl.classList.remove("hidden");
      const supervisor = supFilter.value;
      const pesquisador = pesqFilter.value;
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        labels.push(`${monthNames[targetMes - 1]}/${targetAno.toString().slice(2)}`);
        promises.push(
          supabase.rpc("get_loja_perfeita_data", {
            p_ano: targetAno.toString(),
            p_mes: targetMes.toString(),
            p_supervisor: supervisor || null,
            p_pesquisador: pesquisador || null,
          })
        );
      }
      ...
      const chartData = results.map(res => {
        ...
        return kpis && kpis.avg_score ? parseFloat(kpis.avg_score) : 0;
      });

      loadingEl.classList.add("hidden");

      const ctx = document.getElementById("lojaPerfeitaChart").getContext("2d");
      if (lojaPerfeitaChartInstance) lojaPerfeitaChartInstance.destroy();
    ...
    supFilter.addEventListener("change", loadData);
    pesqFilter.addEventListener("change", loadData);

    await loadData();
  }


  function renderCategoriasDispute(data, activeMetric) {
    if (!data.categorias_disputa || data.categorias_disputa.length === 0) return;

    const categoriasData = data.categorias_disputa;
    const kpiSalty = data.kpi_salty || [];
    const vendedoresCategorias = data.vendedores_categorias || [];

    const btnEls = document.querySelectorAll("#categorias-metric-filters .metric-btn");
    btnEls.forEach((btn) => {
      if (btn.dataset.metric === activeMetric) {
        btn.classList.add("bg-[#fc0100]", "text-white");
        btn.classList.remove("bg-white/5", "text-slate-300");
      } else {
        btn.classList.remove("bg-[#fc0100]", "text-white");
    ...
    let aguiaSalty = 0;
    kpiSalty.forEach((k) => {
      if (k.equipe === "SHARK") sharkSalty = k.clientes_salty;
      if (k.equipe === "ÁGUIA") aguiaSalty = k.clientes_salty;
    });

    const catGroups = {};
    categoriasData.forEach((c) => {
      if (!catGroups[c.categoria]) catGroups[c.categoria] = { SHARK: {}, ÁGUIA: {} };
      catGroups[c.categoria][c.equipe] = c;
    });

    let sharkDominios = 0;
    let aguiaDominios = 0;

    const shareRankingHtml = [];
    const growthRanking = [];

    Object.keys(catGroups).forEach((cat) => {
      const sharkRow = catGroups[cat]["SHARK"] || { fat_atual: 0, ton_atual: 0, pos_atual: 0, fat_trim: 0, ton_trim: 0, pos_trim: 0 };
      const aguiaRow = catGroups[cat]["ÁGUIA"] || { fat_atual: 0, ton_atual: 0, pos_atual: 0, fat_trim: 0, ton_trim: 0, pos_trim: 0 };

      const totalFat = sharkRow.fat_atual + aguiaRow.fat_atual;
      const sharkFatShare = totalFat > 0 ? (sharkRow.fat_atual / totalFat) * 100 : 0;
      const aguiaFatShare = totalFat > 0 ? (aguiaRow.fat_atual / totalFat) * 100 : 0;

      if (sharkFatShare > 50) sharkDominios++;
      if (aguiaFatShare > 50) aguiaDominios++;

      shareRankingHtml.push(`
        <div class="flex items-center text-xs font-semibold">
            <div class="w-48 text-slate-300 uppercase truncate pr-2" title="${cat}">${cat}</div>
            <div class="flex-grow flex rounded overflow-hidden h-3 bg-white/5">
                <div style="width: ${sharkFatShare}%" class="bg-blue-600 h-full text-[9px] flex items-center px-1 text-white">${sharkFatShare > 10 ? sharkFatShare.toFixed(1) + "%" : ""}</div>
                <div style="width: ${aguiaFatShare}%" class="bg-red-600 h-full text-[9px] flex items-center justify-end px-1 text-white">${aguiaFatShare > 10 ? aguiaFatShare.toFixed(1) + "%" : ""}</div>
            </div>
        </div>
      `);

      const calcGrowth = (atual, trim) => (trim > 0 ? ((atual - trim) / trim) * 100 : 0);
      const getScore = (row) => {
        if (activeMetric === "faturamento") return calcGrowth(row.fat_atual, row.fat_trim);
        if (activeMetric === "tonelada") return calcGrowth(row.ton_atual, row.ton_trim);
        if (activeMetric === "posituacao") return calcGrowth(row.pos_atual, row.pos_trim);
        return calcGrowth(row.fat_atual, row.fat_trim) + calcGrowth(row.ton_atual, row.ton_trim) + calcGrowth(row.pos_atual, row.pos_trim);
      };

      growthRanking.push({
        cat,
        sharkScore: getScore(sharkRow),
        aguiaScore: getScore(aguiaRow),
      });
    });

    document.getElementById("categorias-kpis").innerHTML = `
       <div class="card p-3 rounded-lg border-l-4 border-blue-500 text-center">
           <div class="text-[10px] text-blue-300 font-bold uppercase tracking-wider mb-1">Clientes Salty (Shark)</div>
           <div class="text-xl font-bold text-white">${sharkSalty}</div>
       </div>
       <div class="card p-3 rounded-lg border-l-4 border-blue-500 text-center">
           <div class="text-[10px] text-blue-300 font-bold uppercase tracking-wider mb-1">Domínio Categorias (Shark)</div>
           <div class="text-xl font-bold text-white">${sharkDominios}</div>
       </div>
       <div class="card p-3 rounded-lg border-l-4 border-red-500 text-center">
           <div class="text-[10px] text-red-300 font-bold uppercase tracking-wider mb-1">Domínio Categorias (Águia)</div>
           <div class="text-xl font-bold text-white">${aguiaDominios}</div>
       </div>
       <div class="card p-3 rounded-lg border-l-4 border-red-500 text-center">
           <div class="text-[10px] text-red-300 font-bold uppercase tracking-wider mb-1">Clientes Salty (Águia)</div>
           <div class="text-xl font-bold text-white">${aguiaSalty}</div>
       </div>
    `;

    document.getElementById("categorias-share-ranking").innerHTML = shareRankingHtml.join("");

    growthRanking.sort((a, b) => (b.sharkScore + b.aguiaScore) - (a.sharkScore + a.aguiaScore));
    document.getElementById("growth-metric-label").textContent = `Métrica: ${activeMetric.charAt(0).toUpperCase() + activeMetric.slice(1)}`;
    document.getElementById("categorias-growth-ranking").innerHTML = growthRanking.map((g) => `
        <div class="flex items-center text-xs font-semibold">
            <div class="w-32 text-slate-300 uppercase truncate pr-2">${g.cat}</div>
            <div class="flex-grow flex justify-between">
                <div class="${g.sharkScore >= 0 ? "text-emerald-400" : "text-red-400"} w-1/2 text-left">Shark: ${g.sharkScore > 0 ? "+" : ""}${g.sharkScore.toFixed(1)}%</div>
                <div class="${g.aguiaScore >= 0 ? "text-emerald-400" : "text-red-400"} w-1/2 text-right">Águia: ${g.aguiaScore > 0 ? "+" : ""}${g.aguiaScore.toFixed(1)}%</div>
            </div>
        </div>
    `).join("");

    const sortedSellers = [...vendedoresCategorias].sort((a, b) => {
      if (b.categorias_distintas !== a.categorias_distintas) return b.categorias_distintas - a.categorias_distintas;
      return b.faturamento - a.faturamento;
    });

    const renderSeller = (v, index) => `
        <div class="flex justify-between items-center bg-white/5 p-2 rounded">
            <div class="flex items-center gap-3">
                <div class="w-5 h-5 rounded-full bg-[#1a1625] flex items-center justify-center text-[10px] font-bold text-slate-400">${index + 1}</div>
                <div class="text-xs font-semibold text-white uppercase max-w-[120px] truncate" title="${v.vendedor}">${v.vendedor}</div>
            </div>
            <div class="text-right">
                <div class="text-xs font-bold text-white">${v.categorias_distintas} <span class="text-[9px] text-slate-400">Cats</span></div>
                <div class="text-[9px] text-slate-400">${new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(v.faturamento)}</div>
            </div>
        </div>
    `;

    const topShark = sortedSellers.filter((s) => s.equipe === "SHARK").slice(0, 3);
    const topAguia = sortedSellers.filter((s) => s.equipe === "ÁGUIA").slice(0, 3);

    document.getElementById("categorias-sellers-shark").innerHTML = topShark.map(renderSeller).join("");
    document.getElementById("categorias-sellers-aguia").innerHTML = topAguia.map(renderSeller).join("");
  }

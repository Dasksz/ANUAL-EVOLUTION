...
function parseDate(dateString) {
  ...
  if (str.length >= 10) {
    ...
    if (c4 === 45 && str.charCodeAt(7) === 45) {
      if (
        isDigit(str.charCodeAt(0)) &&
        isDigit(str.charCodeAt(1)) &&
        isDigit(str.charCodeAt(2)) &&
        isDigit(str.charCodeAt(3)) &&
        isDigit(str.charCodeAt(5)) &&
        isDigit(str.charCodeAt(6)) &&
        isDigit(str.charCodeAt(8)) &&
        isDigit(str.charCodeAt(9))
      ) {
        const y =
    ...
    // dd/mm/yyyy or dd-mm-yyyy ('/' is 47, '-' is 45)
      ...
      (c2 === 47 || c2 === 45) &&
      (str.charCodeAt(5) === 47 || str.charCodeAt(5) === 45)
    ) {
      if (
        isDigit(str.charCodeAt(0)) &&
        isDigit(str.charCodeAt(1)) &&
        isDigit(str.charCodeAt(3)) &&
        isDigit(str.charCodeAt(4)) &&
        isDigit(str.charCodeAt(6)) &&
        isDigit(str.charCodeAt(7)) &&
        isDigit(str.charCodeAt(8)) &&
        isDigit(str.charCodeAt(9))
      ) {
        ...
        const y =
          (str.charCodeAt(6) - 48) * 1000 +
          (str.charCodeAt(7) - 48) * 100 +
    ...
    reader.onload = (event) => {
      try {
        ...
        if (file.name.endsWith(".csv")) {
          let decodedData;
          try {
            decodedData = new TextDecoder("utf-8", { fatal: true }).decode(
              new Uint8Array(data),
            );
          } catch (e) {
            decodedData = new TextDecoder("iso-8859-1").decode(
              new Uint8Array(data),
            );
          }
...
const processSalesData = (rawData, clientMap, productMasterMap) => {
  return rawData.map((rawRow) => {
    ...
    if (supervisorName.trim().toUpperCase() === "OSÉAS SANTOS OL")
      supervisorName = "OSVALDO NUNES O";
    ...
    if (
      parsedDtPed &&
      parsedDtSaida &&
      (parsedDtPed.getUTCFullYear() < parsedDtSaida.getUTCFullYear() ||
        (parsedDtPed.getUTCFullYear() === parsedDtSaida.getUTCFullYear() &&
          parsedDtPed.getUTCMonth() < parsedDtSaida.getUTCMonth()))
    ) {
      dtPed = dtSaida;
    ...
    const formattedDtPed = parsedDtPed
      ? parsedDtPed.toISOString().split("T")[0]
      : null;
    const formattedDtSaida = parsedDtSaida
      ? parsedDtSaida.toISOString().split("T")[0]
      : null;
    ...
    return {
      ...
      cidade:
        clientInfo.cidade ||
        (rawRow["MUNICIPIO"] ? normalizeCityName(rawRow["MUNICIPIO"]) : null),
      bairro:
        clientInfo.bairro || String(rawRow["BAIRRO"] || "N/A").toUpperCase(),
      qtvenda: qtVenda,
...
function processMetasEstrelas(data, mes, ano) {
  ...
  for (const row of data) {
    ...
    const calcPos =
      parseInt(row["Calibração Pos"] || row["Calibração Pos "], 10) || 0;
    ...
    const calcFoods = parseDecimal(
      row["Calibração Foods"] || row["Calibração Foods "],
    );
    const calcSalty = parseDecimal(
      row["Calibração Salty"] || row["Calibração Salty "],
    );
...
function processLojaPerfeita(filesData, clientCnpjMap, clientsToInsert) {
  ...
  const getPesqVal = (row) => {
    for (const k in row) {
      if (k.toUpperCase().includes("PESQUISADOR"))
        return String(row[k] || "")
          .trim()
          .normalize("NFD")
          .replace(/[\u0300-\u036f]/g, "")
  ...
  combinedForInference.forEach((row) => {
    let cnpjRaw = null;
    for (const k in row) {
      if (k.toUpperCase().includes("CNPJ") || k.toUpperCase().includes("CPF")) {
        cnpjRaw = row[k];
        break;
      }
    }
    ...
    if (!codCli && cnpjClean.length <= 14)
      codCli = clientCnpjMap.get(cnpjClean.padStart(14, "0"));
    if (!codCli && cnpjClean.length <= 11)
      codCli = clientCnpjMap.get(cnpjClean.padStart(11, "0"));

    if (codCli) {
      const pesquisador = getPesqVal(row);
      if (pesquisador) {
        // Find the city for this codCli in clientsToInsert
        const clientInfo = clientsToInsert.find(
          (c) => c.codigo_cliente === codCli,
        );
        if (clientInfo && clientInfo.cidade) {
          if (!researcherCities.has(pesquisador))
            researcherCities.set(pesquisador, new Map());
          const cityCounts = researcherCities.get(pesquisador);
          cityCounts.set(
            clientInfo.cidade,
            (cityCounts.get(clientInfo.cidade) || 0) + 1,
          );
        }
      }
    }
  ...
  combined.forEach((row) => {
    ...
    const pesquisador = String(getVal(row, "Pesquisador") || "")
      .trim()
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace(/\s+/g, "")
    ...
    const notaRaw =
      getVal(row, "Nota Média Total Alavancada") ||
      getVal(row, "Nota Media Total Alavancada") ||
      getVal(row, "Nota Média") ||
      getVal(row, "Nota Media");
    const nota =
      typeof notaRaw === "number"
        ? notaRaw
        : parseFloat(String(notaRaw || "0").replace(",", "."));
...
if (typeof self !== "undefined") {
  self.onmessage = async (event) => {
    ...
    try {
      ...
      self.postMessage({
        type: "progress",
        status: "Filtrando vendas Pepsico e linhas inválidas...",
        percentage: 15,
      });
      const pepsicoFilter = (sale) =>
        String(sale["OBSERVACAOFOR"] || "")
          .trim()
          .toUpperCase() === "PEPSICO";
      ...
      salesCurrYearHistDataRaw = (salesCurrYearHistDataRaw || []).filter(
        combinedFilter,
      );
      salesCurrMonthDataRaw = (salesCurrMonthDataRaw || []).filter(
        combinedFilter,
      );
      ...
      const allSalesRaw = salesPrevYearDataRaw.concat(
        salesCurrYearHistDataRaw,
        salesCurrMonthDataRaw,
      );
      ...
      self.postMessage({
        type: "progress",
        status: "Verificando códigos IBGE...",
        percentage: 18,
      });
      ...
      if (potentialCodes.size > 0) {
        self.postMessage({
          type: "progress",
          status: "Buscando nomes de cidades (IBGE)...",
          percentage: 19,
        });
        try {
      ...
      if (Object.keys(ibgeMap).length > 0) {
        ...
        salesCurrYearHistDataRaw.forEach((r) =>
          replaceIbgeCode(r, "MUNICIPIO"),
        );
      ...
      self.postMessage({
        type: "progress",
        status: "Mapeando cidades pelas vendas...",
        percentage: 19.5,
      });
      ...
      const processedClients = await Promise.all(
        (clientsDataRaw || []).map(async (client) => {
          const codCli = String(client["Código"] || "").trim();
          if (!codCli) return null;

          let rca1 = String(client["RCA 1"] || "");
          const nomeClienteStr = String(
            client["Fantasia"] || client["Cliente"] || "N/A",
          ).toUpperCase();
          const razaoSocialStr = String(
            client["Cliente"] || "N/A",
          ).toUpperCase();
          if (
            nomeClienteStr.includes("AMERICANAS S.A") ||
            razaoSocialStr.includes("AMERICANAS S.A")
          ) {
            rca1 = "AMERICANAS";
          }

          const rawCnpj = client["CNPJ/CPF"] || client["Cpf/Cnpj"] || "";
          const cleanedCnpj = rawCnpj
            ? String(rawCnpj).replace(/[^0-9]/g, "")
            : null;
          ...
          const clientData = {
            ...
            cidade:
              salesCity ||
              String(client["Nome da Cidade"] || client["Cidade"] || "")
                .trim()
                .toUpperCase() ||
              null,
            nomecliente: String(
            ...
            ramo:
              client["Descricao"] &&
              String(client["Descricao"]).trim().toUpperCase() !== "N/A" &&
              String(client["Descricao"]).trim().toUpperCase() !== "N/D"
                ? String(client["Descricao"]).trim().toUpperCase()
                : null,
            ramo_atividade:
              client["Ramo Atividade"] &&
              String(client["Ramo Atividade"]).trim().toUpperCase() !== "N/A" &&
              String(client["Ramo Atividade"]).trim().toUpperCase() !== "N/D"
                ? String(client["Ramo Atividade"]).trim().toUpperCase()
                : null,
            ultimacompra: ultimaCompra ? ultimaCompra.toISOString() : null,
            bloqueio: String(client["Bloqueio"] || "")
              .trim()
              .toUpperCase(),
          };
      ...
      self.postMessage({
        type: "progress",
        status: "Unificando códigos de produtos...",
        percentage: 25,
      });
      ...
      allSalesRaw.forEach((row) => {
        ...
        if (desc && productCode && desc !== "N/D" && desc !== "N/A") {
          ...
          if (!descToCodeMap.has(desc)) {
            descToCodeMap.set(desc, {
              code: productCode,
              time: saleTime,
              dtStr: String(dtPedRaw),
            });
          } else {
      ...
      productsDataRaw.forEach((prod) => {
        ...
        let isRecent = false;
        if (dtCadastro) {
          isRecent = dtCadastro >= thirtyDaysAgo;
        }

        if (!activeProductCodes.has(productCode) && !isRecent) return;
        ...
        dimProducts.set(productCode, {
          ...
          dt_cadastro: dtCadastro
            ? dtCadastro.toISOString().split("T")[0]
            : null,
        });
      });
      ...
      for (const row of allSalesRaw) {
        ...
        if (!existingEntry) {
          rcaInfoMap.set(codusur, {
            ...
            CODSUPERVISOR: codSupervisor || "N/A",
          });
        } else {
      ...
      self.postMessage({
        type: "progress",
        status: "Processando e Reatribuindo vendas...",
        percentage: 50,
      });

      const reattributeSales = (salesData, isCurrMonth = false) => {
        const balcaoSpecialClients = new Set([
          "6421",
          "7706",
          "9814",
          "11405",
        ...
        return salesData.map((sale) => {
          ...
          const isBalcaoException =
            originalCodCli === "9569" && originalCodUsur === "53";
          // Case B: Balcao List
          ...
          const clientName = clientData
            ? clientData.nomeCliente.toUpperCase()
            : rawName;
          const clientRazao = clientData
            ? clientData.razaosocial.toUpperCase()
            : "";
          const isAmericanas =
            clientName.includes("AMERICANAS") ||
            clientName.includes("AMERICANAS S.A") ||
            clientRazao.includes("AMERICANAS") ||
            clientRazao.includes("AMERICANAS S.A");
          ...
          if (isBalcaoException) {
                ...
                vendorCities.forEach((city) => {
                  ...
                  if (cityCounts) {
                    cityCounts.forEach((count, superv) => {
                      supervisorAggregatedCounts.set(
                        superv,
                        (supervisorAggregatedCounts.get(superv) || 0) + count,
                      );
                    });
                  }
                });
            ...
            if (!targetSupervisor) {
              targetSupervisor =
                cityPredominantSupervisorMap.get(municipio) || "INATIVOS";
            }

            const sanitizedSupervisor = targetSupervisor.replace(
              /[^A-Z0-9]/g,
              "",
            ); // Simple sanitization for code

            newSale["SUPERV"] = targetSupervisor;
            newSale["CODUSUR"] = `INAT_${sanitizedSupervisor}`;
            newSale["NOME"] = `INATIVOS ${targetSupervisor}`;

            const targetSupervisorCode =
              supervisorNameMap.get(targetSupervisor);
            ...
            const targetVendorCode =
              rca1 || String(newSale["CODUSUR"] || "").trim();
            ...
            if (targetVendorCode && rcaInfoMap.has(targetVendorCode)) {
              ...
              if (targetVendorCode === "190") {
                ...
                newSale["SUPERV"] =
                  supervisorCodeMap.get("12") || "SUPERVISOR 12";
              } else {
                newSale["SUPERV"] = vendorInfo.SUPERV;
                if (
                  vendorInfo.CODSUPERVISOR &&
                  vendorInfo.CODSUPERVISOR !== "N/A"
                ) {
      ...
      const reattributedPrevYear = reattributeSales(
        salesPrevYearDataRaw,
        false,
      );
      const reattributedCurrYearHist = reattributeSales(
        salesCurrYearHistDataRaw,
        false,
      );
      const reattributedCurrMonth = reattributeSales(
        salesCurrMonthDataRaw,
        true,
      );

      const processedPrevYear = processSalesData(
        reattributedPrevYear,
        clientMap,
        productMasterMap,
      );
      const processedCurrYearHist = processSalesData(
        reattributedCurrYearHist,
        clientMap,
        productMasterMap,
      );
      const processedCurrMonth = processSalesData(
        reattributedCurrMonth,
        clientMap,
        productMasterMap,
      );
      ...
      const finalProductStock = salesCurrMonthFile
        ? Array.from(productStockMap.values())
        : null;

      // --- Collect Dimensions (Supervisors, Vendors, Providers) ---
      self.postMessage({
        type: "progress",
        status: "Extraindo dimensões (Supervisores, Vendedores)...",
        percentage: 80,
      });
      ...
      const chunkData = async (salesArray, isHistory = false) => {
        ...
        for (const sale of salesArray) {
          ...
          if (!dateStr && newSale.dtped instanceof Date)
            dateStr = newSale.dtped.toISOString();
      ...
      const historyChunks =
        salesPrevYearFile || salesCurrYearFile
          ? await chunkData(
              [...processedPrevYear, ...processedCurrYearHist],
              true,
            )
          : null;
      const detailedChunks = salesCurrMonthFile
        ? await chunkData(processedCurrMonth, false)
        : null;

      self.postMessage({
        type: "progress",
        status: "Preparando dados para envio...",
        percentage: 90,
      });

      // Helper to convert Map to Array of Objects
      const mapToObjArray = (map) =>
        Array.from(map.entries()).map(([codigo, nome]) => ({ codigo, nome }));
      ...
      const finalProducts = productsFile
        ? Array.from(dimProducts.entries()).map(([codigo, val]) => ({
            codigo,
            descricao: val.descricao,
            codfor: val.codfor,
            qtde_embalagem_master: val.qtde_embalagem_master,
      ...
      if (innovationsDataRaw && innovationsDataRaw.length > 0) {
        finalInnovations = innovationsDataRaw
          .map((item) => ({
            codigo: String(item["Codigo"] || item["codigo"] || "").trim(),
            inovacoes: String(
              item["Inovacoes"] || item["inovacoes"] || "",
            ).trim(),
          }))
      ...
      if (
        (nota1DataRaw && nota1DataRaw.length > 0) ||
        (nota2DataRaw && nota2DataRaw.length > 0)
      ) {
        ...
        if (existingClientsMap && existingClientsMap.length > 0) {
          for (const client of existingClientsMap) {
            ...
            if (codCli && cnpjRaw) {
              ...
              if (typeof cnpjRaw === "number" && cnpjStr.includes("e")) {
                try {
                  cnpjStr = cnpjRaw.toLocaleString("fullwide", {
                    useGrouping: false,
                  });
                } catch (e) {}
              }
        ...
        if (clientsDataRaw && clientsDataRaw.length > 0) {
          for (const client of clientsDataRaw) {
            const codCli = String(
              client["Código"] || client["Codigo"] || client["codigo"] || "",
            ).trim();
            ...
            const cnpjRaw =
              getClientVal(client, "CNPJ") || getClientVal(client, "CPF");
            if (codCli && cnpjRaw) {
              let cnpjStr = String(cnpjRaw);
              if (typeof cnpjRaw === "number" && cnpjStr.includes("e")) {
                try {
        ...
        const notaResult = processLojaPerfeita(
          [nota1DataRaw, nota2DataRaw],
          clientCnpjMap,
          clientsToInsert,
        );
      ...
      if (metaEstrelasDataRaw && metaEstrelasDataRaw.length > 0) {
        self.postMessage({
          type: "progress",
          status: "Processando Metas Estrelas...",
          percentage: 95,
        });
        finalMetaEstrelas = processMetasEstrelas(
          metaEstrelasDataRaw,
          metaEstrelasMes,
          metaEstrelasAno,
        );

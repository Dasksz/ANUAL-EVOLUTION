...
let estrelasSelectedCategorias = [];
    ...
    document.querySelectorAll('.side-nav-item').forEach(link => {
        link.addEventListener('click', () => {
            if (window.innerWidth < 1024) { // Assuming mobile/tablet view mostly uses this, but let's always close
                closeSideMenu();
            } else {
                closeSideMenu(); // Instructed to always close on click
            }
        });
    });
    ...
    // Global Export Buttons in Side Menu
    const globalExportExcelBtn = document.getElementById('export-excel-btn');
    const globalExportPdfBtn = document.getElementById('export-pdf-btn');

    if (globalExportExcelBtn) {
        globalExportExcelBtn.addEventListener('click', () => {
            closeSideMenu();
            // Try to find the active view's specific export function or button
            // If we are in the boxes view:
            if (!boxesView.classList.contains('hidden') && typeof exportBoxesTable === 'function') {
                exportBoxesTable('excel');
            } else {
                 window.showToast('info', 'Exportação para esta tela será implementada em breve.');
            }
        });
    }

    if (globalExportPdfBtn) {
        globalExportPdfBtn.addEventListener('click', () => {
            closeSideMenu();
            // Try to find the active view's specific export function or button
            // If we are in the boxes view:
            if (!boxesView.classList.contains('hidden') && typeof exportBoxesTable === 'function') {

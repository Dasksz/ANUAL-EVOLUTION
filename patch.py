...
with open('src/js/presentation.js', 'r') as f:
    content = f.read()

replacement = """        // "Rede" is now conceptually "Atacado"
...
pattern = r'\s*// "Rede" is now conceptually "Atacado"\s*const topAtacado = redesData\.sort\(\(a,b\) => b\.fat_atual - a\.fat_atual\)\[0\] \|\| redesData\[0\];\s*container\.innerHTML = `\s*<div class="presentation-card bg-gradient-to-br from-fuchsia-900/40 to-transparent border-fuchsia-500/30">\s*<h3 class="text-lg font-bold text-white mb-4 border-b border-fuchsia-500/30 pb-2">Top Atacado: \$\{topAtacado\.dimension\}</h3>.*?<div class="presentation-card">\s*<h3 class="text-sm font-bold text-slate-300 mb-4 border-b border-white/10 pb-2">Outros Atacados</h3>\s*<div class="space-y-3 max-h-48 overflow-y-auto custom-scrollbar pr-2">\s*\$\{redesData\.filter\(r => r\.dimension !== topAtacado\.dimension\)\.map\(r => `\s*<div class="flex justify-between items-center bg-black/20 p-2 rounded">\s*<span class="text-sm font-medium text-slate-300 truncate w-32" title="\$\{r\.dimension\}">\$\{r\.dimension\}</span>\s*<span class="text-sm text-white">\$\{formatCurrency\(r\.fat_atual\)\}</span>\s*<span class="text-xs">\$\{renderVarBadge\(r\.fat_atual, r\.fat_trim\)\} vs Trim, \$\{renderVarBadge\(r\.fat_atual, r\.fat_ant\)\} vs Ano</span>\s*</div>\s*`\)\.join\(''\)\}\s*</div>\s*</div>\s*`;'

new_content = re.sub(pattern, replacement, content, flags=re.DOTALL)

with open('src/js/presentation.js', 'w') as f:
    f.write(new_content)

...
## 2026-08-08 - Fix Query Tuner Regressions
**Learning:** When performing string replacements across SQL files for performance optimizations (like adding `ANY()` or table aliases), it's crucial to evaluate whether the alias (e.g., `s.` or `dp.`) is actually available in every single CTE or query path that uses the dynamic filter. Furthermore, comma-separated columns inside an `ANY()` clause (`col1, col2 = ANY(...)`) generates syntax errors.
**Action:** Always test the generated dynamic SQL paths (e.g. fast path vs slow path) and use less aggressive, more context-aware replacements (like Perl with bounded line numbers) instead of global sed.

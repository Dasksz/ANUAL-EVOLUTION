...
## 2024-05-19 - [Foods KPI Optimization]
**Optimization in SQL Query**: When updating the `positivacao_foods` logic to count clients that exclusively bought from supplier `1119` without any other purchases, optimized the query by using an inline `NOT EXISTS` combined with an `EXISTS` check rather than using computationally heavy `GROUP BY` and `bool_and` window functions. This prevents heavy aggregation over raw sales arrays, avoiding potential memory exhaustion and query timeouts when dealing with the `target_sales` table.

...
## 2026-08-19 - Fix get_boxes_dashboard_data Year Bug
 **Learning:** When writing dynamic SQL for both a specific month and a full year mode, passing current/previous year constants for date generation (e.g., `make_date(%L, 1, 1)`) while mixing them with a dynamic end bound (e.g. `< (make_date(%L, %L, 1) + 1 month)`) can easily cause mismatch bugs (like querying last year's data for current year stats).
 **Action:** Instead of dynamically generating start/end bounds directly inside the `EXECUTE format` string via `EXTRACT(YEAR)` or `make_date`, pre-calculate absolute `v_prod_start_date` and `v_prod_end_date` variables in the main PL/pgSQL block and inject them once as `>= %L::date` and `< %L::date`. This simplifies the format string, enables easier indexing (sargable bounds), and mathematically guarantees alignment across CTEs.
\n## 2026-08-20 - Fix get_boxes_dashboard_data Year Bug\n **Learning:** When injecting variables into `EXECUTE format()` strings for PostgreSQL date comparisons using `%L::date` (or similar date placeholders), ensure you pass valid date objects/strings from PL/pgSQL (e.g., via `make_date(v_current_year, 1, 1)`). Passing integer variables directly to a `%L::date` placeholder causes 'invalid input syntax for type date' casting errors.\n **Action:** Replaced direct passing of integer variables (`v_current_year`) with constructed date functions (`make_date(v_current_year, 1, 1)`) in the `EXECUTE format` bindings for sargable date bounds in `get_boxes_dashboard_data`.

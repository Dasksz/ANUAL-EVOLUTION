...
## 2026-06-10 - Uploader Data Chunks Verification

**Learning:** When checking if data chunks exist before executing long-running RPC background processes (e.g., Supabase summarize caches), checking `.length` on objects fails silently (returns `undefined`, falsy), which inadvertently skips critical update paths causing large API timeouts later as reads hit raw tables instead of cached summaries.
**Action:** Always use `Object.keys(obj).length > 0` to check for emptiness when the container is a key-value object map, not an array.

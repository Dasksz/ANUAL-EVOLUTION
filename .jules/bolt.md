...
## 2024-06-23 - Sync Stale Data Only
**Learning:** Checking local DB timestamp offsets (e.g. `> 60 days`) prevents excessive unneeded network requests to external APIs, keeping initialization fast.
**Action:** When integrating third party metrics, always cache and only request updates if staleness threshold is exceeded.

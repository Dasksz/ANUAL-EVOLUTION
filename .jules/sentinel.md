...
## $(date +%Y-%m-%d) - Securing Agent Materialized View permissions
**Vulnerability:** A materialized view `n8n_agent_view` had its read permissions openly granted to `anon` and `authenticated` roles via `GRANT SELECT ... TO authenticated, anon;`, causing a security risk and prompting Supabase to flag it as 'UNRESTRICTED'. This allows unauthorized scraping of compiled and joined data metrics by anyone with the API URL and anon key.
**Learning:** Supabase tags objects available to `anon` and `authenticated` roles as 'UNRESTRICTED' if they lack RLS (Row-Level Security). Materialized views do not support RLS directly in Postgres. When intended for exclusive consumption by trusted backend agents (like n8n) that utilize the `service_role` key, these objects should strictly limit access.
**Prevention:** Remove `anon` and `authenticated` access to backend-exclusive materialized views via `REVOKE ALL ... FROM anon, authenticated;` and explicitly `GRANT` access only to the `service_role` role.

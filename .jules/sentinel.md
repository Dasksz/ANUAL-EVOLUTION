...
## 2024-06-27 - Force literal agent extraction of pre-formatted DB strings
Modified the system prompt in `N8N PRIME/AGENTE ELMA V3.json` to explicitly forbid the AI agent from altering, summarizing, translating, or omitting items when displaying the `texto_pronto_para_enviar` database string. LLMs with tool access often try to independently filter arrays or truncate lists unless sternly instructed with rules (e.g. `REGRA DE OURO`). This is a critical security and reliability improvement to prevent hallucinations masking real backend SQL results.
